...
| M6 end-to-end | **tunnel up** (2026-08-26); carrier creds pending | ngrok wired for this instance: user's account, static free domain = `PUBLIC_URL` in `.env` (kept out of the repo), agent ≥3.20 at `.local\ngrok\ngrok.exe` (`NGROK_PATH`; winget's 3.3.1 is too old for free accounts). Launcher starts the tunnel on every launch; public `/health` returns API JSON for webhook-style clients (free tier shows a one-time browser interstitial to human visitors). **The application survey is live on the public site** (2026-08-26): `GET /` links to `/apply` → property picker → the same 11-question form as the SMS path, `POST /apply/:propertyId` creates a `channel:"web_link"` application (normalized typed phone, encrypted DOB, 30-day duplicate policy, owner forwarding); dashboard filters/badges and the admin surveys + owner views include `web_link`; covered by `survey-apply.test.ts` (7 tests). Still needed: Telnyx/Twilio/Stripe/Plaid keys + a phone number for this instance for the SMS/voice channels |
